...
if prompt := st.chat_input():
    with st.spinner("Processing..."):
        ...
        events = graph.stream([HumanMessage(content=prompt)], {"configurable": {"thread_id": "2"}})
        for i, step in enumerate(events):
            node, output = next(iter(step.items()))
            if node == "draft":
                print(f'node: {node}')
                first_answer = "***First Answer***:\n\n" + json.loads(output.additional_kwargs['tool_calls'][0]['function']['arguments'])['answer']['answer'] + "\n\nReferences:\n"
                reference = json.loads(output.additional_kwargs['tool_calls'][0]['function']['arguments'])['answer']['references']
                for ref in reference:
                    first_answer += f"\n{ref}\n"
                reflection = json.loads(output.additional_kwargs['tool_calls'][0]['function']['arguments'])['reflection']
                first_answer += f"\n\nReflection:\n\nMissing: {reflection['missing']}\n\nSuperfluous: {reflection['superfluous']}"
                print(f"First Answer:\n {first_answer}")
                st.chat_message("ai").write(first_answer)
                msgs.add_ai_message(first_answer)
            if node == "revise":
                ...
                st.chat_message("ai").write(revised_answer)
                msgs.add_ai_message(revised_answer)
